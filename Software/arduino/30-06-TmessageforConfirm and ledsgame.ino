...
void setup() {
  ...
  // Execute tilight if Switch is OFF
...
const unsigned long tlight_high = 50; // 100 ms on
const unsigned long tlight_low = 50;  // 100 ms off
      ...
      if (rcvd) { // More than zero bytes received
          for (uint16_t i = 0; i < rcvd; i++) {
            ...
            if (data == '>' && datanext == '>' && insideMarkers){ 
              ...
              if(buffer = "<<T>>"){
                ButtonSwitchServos();
                //Serial.println("Received <<T>>, repositioning servos to initial state");
                // Wait for the switch to be HIGH before continuing
                while (digitalRead(switchPin) == LOW) {
                    Tilight();
                }
                //sendConfirmation();
